...
public class TolicTest {
	...
	public static void main(String[] args) {
		...
		header.setFunctioncode("YTJ1011_4");
		header.setErrCode("0");
		header.setErrMsg("");
		...
		// 主键（引用检查记录表ID）
		tolicVo.setExamID("1564859");//引用检查记录表ID
		tolicVo.setSystolic("80");//收缩压
		tolicVo.setDiastolic("120");//舒张压
		tolicVo.setIdNumber("131182199104265201");;// 个人档案Id
		tolicVo.setIdNumber("131182199104265201");// 被检查者身份证号
		tolicVo.setName("韩");// 姓名
		tolicVo.setPhone("13845216598");// 联系方式
		tolicVo.setHealthNO("1358245895");// 健康卡号
		tolicVo.setOrganName("12352");// 机构
		tolicVo.setCreateName("刘晓敏");// 创建者
		tolicVo.setCreateTime("2019-02-01");// 创建时间
		tolicVo.setUpdateName("刘丽");// 修改者
		tolicVo.setUpdateTime("2019-02-01");// 修改时间
		tolicVo.setDataResType("SX0374");// 数据源类型
		tolicVo.setSSupplierCode("1325486595X");// 设备供应商
		tolicVo.setSMachineCode("01^1321251232");// 设备编码
		tolicVo.setIsSuccess("0");// 是否上传成功
		tolicVo.setUploadTime("2019-02-01");// 上传时间
		tolicVo.setErrReason("数据不全");// 上传失败原因
		
		Tolic tolic = ConvertObject.convertToTolic(tolicVo);
		list.add(tolic);
